...
dt_time_vector <- 60*60
number_days <- 7
time_vector <- seq(0,number_days*24*60*60,dt_time_vector)
time_vector_24 <- c(rep(seq(0,1*23*60*60,dt_time_vector),number_days),86400)
...
over_time[,"time"] <- rep(time_vector_24,dev_N)
...
for (dev in 1:dev_N){
  ...
  for (i in 1:time_vector_N){
    ...
    #}
    if (usage_freq[length(usage_freq)] >61){
      usage_freq[length(usage_freq)] <- 61
    }
    
    usage_time <- c(usage_time, sum(sub_screen_time$time_diff))
    if (usage_time[length(usage_time)] >60*60){
      usage_time[length(usage_time)] <- 60*60
    }
    #print (position)
  }
