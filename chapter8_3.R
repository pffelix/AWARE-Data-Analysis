...
# usage time low/high
AWARE <- setDT(AWARE)[, usage_time_hilo := sum(usage_time[variable == "esm_boredom_stress"]), by = id]
# usage frequency low/high
AWARE <- setDT(AWARE)[, usage_freq_hilo := sum(usage_freq[variable == "esm_boredom_stress"]), by = id]
...
#window_0_1
# usage time low/high
AWAREsub$usage_time_hilo_0_1 <-  as.integer(ifelse(AWAREsub$usage_time_hilo<=median(unique(AWAREsub$usage_time_hilo),na.rm=TRUE),1,0))
# usage frequency low/high
AWAREsub$usage_freq_hilo_0_1 <-  as.integer(ifelse(AWAREsub$usage_freq_hilo<=median(unique(AWAREsub$usage_freq_hilo),na.rm=TRUE),1,0))
...
AWAREsub$third <- AWAREsub$usage_time_hilo_0_1
...
# high smartphone usage group : third=0
id_vec <- c()
x_vec <- c()
y_vec <- c()
for (i in cfs$id[cfs$third==0]){
...
predl<-fix_Intercept + fix_indepcw*process$indep[process$third==0]
spag_data <- data.frame(Participant=id_vec,x_v=x_vec,y_v=y_vec)
spag_data_pre <- data.frame(Participant=51,x_v=process$indep[process$third==0],y_v=predl)
y_max <-max(spag_data$y_v)
mytheme <- 
p1 <- ggplot() +
  geom_line(data=spag_data,aes(x=x_v, y=y_v, group = Participant, colour = Participant),size=0.5)+
  geom_line(data=spag_data_pre, aes(x=x_v, y=y_v,size="solid"),colour="red")+
  #theme_grey()+
  ...
  scale_y_continuous(expand=c(0,0), limits=c(0,y_max), breaks = seq(-20,20,1)) +
...
# low smartphone usage group : third=1 
id_vec <- c()
x_vec <- c()
y_vec <- c()
for (i in cfs$id[cfs$third==1]){
...
spag_data <- data.frame(Participant=id_vec,x_v=x_vec,y_v=y_vec)
predh<-(fix_Intercept + fix_third) + (fix_indepcw + fix_indepcw_third)*process$indep[process$third==1]  
spag_data_pre <- data.frame(Participant=51,x_v=process$indep[process$third==1],y_v=predh)
